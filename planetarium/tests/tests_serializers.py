...
class ShowSessionSerializerTests(APITestCase):
    def setUp(self):
        self.show = AstronomyShow.objects.create(title="Space Journey", description="A journey through space")
        self.dome = PlanetariumDome.objects.create(name="Main Dome", rows=10, seats_in_row=10)
        self.session = ShowSession.objects.create(
            astronomy_show=self.show, planetarium_dome=self.dome, show_time="2024-01-01T10:00:00Z"
        )
...
class TicketSerializerTests(APITestCase):
    def setUp(self):
        ...
        self.show = AstronomyShow.objects.create(title="Moon Mission", description="Explore the moon")
        self.dome = PlanetariumDome.objects.create(name="Lunar Dome", rows=8, seats_in_row=10)
        self.session = ShowSession.objects.create(
            astronomy_show=self.show, planetarium_dome=self.dome, show_time="2024-01-01T12:00:00Z"
        )
        self.reservation = Reservation.objects.create(user=self.user)
        self.ticket = Ticket.objects.create(show_session=self.session, reservation=self.reservation, row=1, seat=1)

    def test_ticket_list_serializer(self):
        serializer = TicketListSerializer(self.ticket)
        self.assertEqual(serializer.data['show_session']['astronomy_show'], self.show.title)
...
class ReservationSerializerTests(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(email="user@test.com", password="password")
        self.reservation = Reservation.objects.create(user=self.user)
        self.show = AstronomyShow.objects.create(title="Mars Landing", description="Landing on Mars")
        self.dome = PlanetariumDome.objects.create(name="Mars Dome", rows=10, seats_in_row=10)
        self.session = ShowSession.objects.create(
            astronomy_show=self.show, planetarium_dome=self.dome, show_time="2024-01-01T14:00:00Z"
        )
        self.ticket = Ticket.objects.create(show_session=self.session, reservation=self.reservation, row=1, seat=2)
